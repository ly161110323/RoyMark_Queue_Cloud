...
function trClick() {
    ...
    $('#itemResultTable tbody').on('click', 'tr', function () {
        ...
        $("#caseAreaLs").val(dataId);
        $("#caseAreaName").val($(this).find("td:eq(2)").text());
        $("#caseAreaId").val($(this).find("td:eq(3)").text());
        $("#caseAreaPrint").val($(this).find("td:eq(4)").text());
        $("#caseAreaOrderNo").val($(this).find("td:eq(5)").text());
        $("#areaName").val($(this).find("td:eq(7)").text());
        $("#areaLs").val($(this).find("td:eq(6)").text());

        $("#hallareaLs").val($(this).find("td:eq(8)").text());
        var caseareaIsurl=$(this).find("td:eq(9)").text();
        if(caseareaIsurl==null || caseareaIsurl=="" || caseareaIsurl=="0" )
        {
            $("#allYes").iCheck('uncheck');
            $("#allNo").iCheck('check');
        }
        else
        {
            $("#allYes").iCheck('check');
            $("#allNo").iCheck('uncheck');
        }

        $("#caseareaUrlstr").val($(this).find("td:eq(10)").text());
...
//获取表单数据，将表单以Ajax形式提交
        var params = {
            "areaLs":$("#Area_Ls").val(),
            "caseareaName": $("#caseAreaName").val(),
            "caseareaId": $("#caseAreaId").val(),
            "caseareaPrint": $("#caseAreaPrint").val(),
            "caseareaOrderno": $("#caseAreaOrderNo").val(),
            "caseareaIsurl": $('input[name="caseareaIsurl"]:checked').val(),
            "caseareaUrlstr": $("#caseareaUrlstr").val()
        }

        var rootPath = getWebRootPath();
        var url = rootPath + "/QueueCasearea/insert";
...
//若效验通过，则获取表单数据，发送修改请求
        ...
        var rootPath = getWebRootPath();
        var url = rootPath + "/QueueCasearea/update";
//获取表单数据
        var params = {
            "areaLs":$("#Area_Ls").val(),
            "caseareaName": $("#caseAreaName").val(),
...
//循环获取每个事项的流水号
            ...
            var rootPath = getWebRootPath();
            var url = rootPath + "/QueueCasearea/delete";
...
//判断是否有重复的
    ...
    var selectName = $("#caseAreaName").val();
    var selectLs = $("#caseAreaLs").val();
    var isExit = false;

    if($("#caseAreaName").val().trim()==""){
        layer.alert("区域名称不能为空！")
        return false;
    }

    if($("#caseAreaId").val().trim()==""){
        layer.alert("区域编号不能为空！")
        return false;
    }
    if($("#caseAreaOrderNo").val().trim()==""){
        layer.alert("顺序号不能为空！")
        return false;
    }
    // var hallAreaLs=$("#hallareaLs").val();
    // if(hallAreaLs==0)
    // {
    //     layer.alert("所属政务中心不能为空！")
    //     return false;
    // }
    ...
    $trs.each(function(index,element){
        ...
        var inputAreadId=$("#caseAreaId").val();
