...
<script type="text/javascript">
	var roleLs = "";
	var roleName = "";
	var areaLs="";//加入所属大厅编号 add by wfz20200119
	var permissionsType = "";
	var allCheckedFlag = "";
	...
	function editRolePermission() {
		...
		var menuLss = "";//拼接字符串
		roleLs = getUrlParams("roleLs");
		areaLs= getUrlParams("areaLs");
		roleName = decodeURI(getUrlParams("roleName"));
		permissionsType = getUrlParams("permissionsType");
		for (var i = 0; i < lgh; i++) {
			menuLss += nodes[i].id+",";
		}
		...
		$
				.ajax({
					type : 'POST',
					url : '${ctx}/QueueRolepermissions/updateRoleMenuPermission',
					data : {
						"roleLs" : roleLs,
						"menuLss" : menuLss,
						"areaLs":areaLs,
					...
					success : function(data) {
						if (data.result == "ok") {
							window.parent.closeFramDialog("菜单权限设置成功！");
						}
						if (data.result == "no") {
							window.parent.closeFramDialog("菜单权限设置失败！");
						}
	...
	$(document).ready(function() {
		roleLs = getUrlParams("roleLs");
        areaLs = getUrlParams("areaLs");
		roleName = getUrlParams("roleName");
		permissionsType = getUrlParams("permissionsType");
		jQuery.browser = {};
		(function() {
	...
	function getTree() {
		$.ajax({
			url : "${ctx}/QueueRolepermissions/roleMenuPermissionSet",
			dataType : "json",
			type : "post",
			data : {
				"roleLs" : roleLs,
