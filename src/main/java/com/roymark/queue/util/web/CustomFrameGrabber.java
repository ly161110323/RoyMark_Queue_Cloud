...
public class CustomFrameGrabber extends FrameGrabber {
    ...
    public static CustomFrameGrabber createDefault(File deviceFile) throws Exception {
        return new CustomFrameGrabber(deviceFile);
    }

    public static CustomFrameGrabber createDefault(String devicePath) throws Exception {
        return new CustomFrameGrabber(devicePath);
    }

    public static CustomFrameGrabber createDefault(int deviceNumber) throws Exception {
        throw new Exception(CustomFrameGrabber.class + " does not support device numbers.");
    }
        ...
        if (loadingException != null) {
            ...
            try {
                ...
                if (var1 instanceof Exception) {
                    ...
                    throw loadingException = new Exception("Failed to load " + CustomFrameGrabber.class, var1);
                }
            }
        }
    }

    public CustomFrameGrabber(File file) {
        this(file.getAbsolutePath());
    }

    public CustomFrameGrabber(String filename) {
        this.filename = filename;
        this.pixelFormat = -1;
        this.sampleFormat = -1;
    }

    public CustomFrameGrabber(InputStream inputStream) {
        this(inputStream, 2147483639);
    }

    public CustomFrameGrabber(InputStream inputStream, int maximumSize) {
        ...
        if (this.oc == null) {
            ...
            if (frameTypesToSeek != null) {
                ...
                if (has_video || has_audio) {
                    if (frameTypesToSeek.contains(Frame.Type.VIDEO) && !has_video || frameTypesToSeek.contains(Frame.Type.AUDIO) && !has_audio) {
            ...
            if (this.format != null && this.format.length() > 0 && (f = avformat.av_find_input_format(this.format)) == null) {
                ...
                if (this.inputStream != null) {
                    if (readCallback == null) {
                        readCallback = new CustomFrameGrabber.ReadCallback();
                    }
                    if (seekCallback == null) {
                        seekCallback = new CustomFrameGrabber.SeekCallback();
                    }
    ...
    static class SeekCallback extends Seek_Pointer_long_int {
        ...
        public long call(Pointer opaque, long offset, int whence) {
            try {
                ...
                long remaining;
                long n;
                is = (InputStream)inputStreams.get(opaque);
                size = 0L;
                label64:
                switch(whence) {
                    case 0:
                    ...
                    case 2:
                        ...
                        while(true) {
                            remaining = is.skip(9223372036854775807L);
                            if (remaining == 0L) {
                                offset += size;
                                is.reset();
                                break label64;
                    ...
                    case 65536:
                        ...
                        while(true) {
                            n = is.skip(9223372036854775807L);
                            if (n == 0L) {
                                is.reset();
    ...
    static class ReadCallback extends Read_packet_Pointer_BytePointer_int {
        ...
        public int call(Pointer opaque, BytePointer buf, int buf_size) {
            try {
                ...
                InputStream is = (InputStream)inputStreams.get(opaque);
